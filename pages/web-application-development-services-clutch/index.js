...
const WebApplicationDevelopmentServicesClutch = (props) => {
...
export default WebApplicationDevelopmentServicesClutch;
...
export async function getStaticProps() {
  ...
  const response = await axiosInstance.get(
    "/api/paid-campaigns?filters[slug][$eq]=web-application-development-services-clutch&populate=hero_section.button,hero_section.image,counter_component.count,teams_we_offer.RoadBox,form.leftside_award.images,form.button,clutch_widget,development_process.image,clientele_brand_section.brand_logo.image,long_cta.image,problem_solution.RoadBox,services.white_box.image,display,how_we_work.image,short_cta.button,client_segment.ClientSegment_box,services_another.white_box.image,engagement_model.Engagement_Box,case_study.image,technology_stack.image,why_choose_maruti_techlabs.white_box.image,data_protection.image,data_protection.dataprotection_box.image,awards_recognitions.images,our_people_our_strength.image,our_people_our_strength.ourpeople_box.image,clutch_review.image,clutch_review.button,resource_library.rl_box.image,resource_library.link,other_services.otherservice_box,faq.button,faq.faqitems,client_testimonial_video.button,client_testimonial_video.video_link.image,client_testimonial_video.variant"
  );
  return {
    props: {
      ...
      teamsweOffer: response?.data[0]?.attributes?.teams_we_offer,
