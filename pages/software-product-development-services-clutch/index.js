...
const SoftwareProductDevelopmentServicesClutch = (props) => {
...
export default SoftwareProductDevelopmentServicesClutch;
...
export async function getStaticProps() {
  ...
  const response = await axiosInstance.get(
    "/api/paid-campaigns?filters[slug][$eq]=software-product-development-services-clutch&populate=hero_section.button,hero_section.image,counter_component.count,form.leftside_award.images,form.button,clutch_widget,clientele_brand_section.brand_logo.image,long_cta.image,problem_solution.RoadBox,services.white_box.image,short_cta.button,services_another.white_box.image,case_study.image,case_study_other.image,technology_stack.image,awards_recognitions.images,resource_library.rl_box.image,resource_library.link,faq.button,faq.faqitems,client_testimonial_video.button,client_testimonial_video.video_link.image,client_testimonial_video.variant,testimonial_banner"
  );
  ...
  return {
    props: {
      ...
      testimonialBanner: response?.data[0]?.attributes?.testimonial_banner,
